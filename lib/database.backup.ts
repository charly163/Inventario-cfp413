...
function getDefaultAppSettings(): AppSettings {
  return {
    ...
    categories: ['EQUIPAMIENTO', 'HERRAMIENTA', 'INSUMO', 'MOBILIARIO', 'OTROS', 'UTENSILIO DE COCINA'],
    sources: ['COMPRA', 'DONACION', 'TRASLADO', 'OTRO'],
    teachers: [], // Ahora se cargarán dinámicamente desde la tabla teachers
    locations: ['ALMACEN', 'AULA', 'OFICINA', 'TALLER']
  };
}
...
const updateTeacher = async (id: string, updates: Partial<any>): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('teachers')
      .update(updates)
...
const addCategory = async (name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('categories')
      .insert([{ name }])
...
const updateCategory = async (id: string, name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('categories')
      .update({ name })
...
const addLocation = async (name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('locations')
      .insert([{ name }])
...
const updateLocation = async (id: string, name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('locations')
      .update({ name })
...
const getSources = async (): Promise<any[]> => {
  try {
    const { data, error } = await supabase
      .from('sources')
      .select('*')
...
const addSource = async (name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('sources')
      .insert([{ name }])
...
const updateSource = async (id: string, name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('sources')
      .update({ name })
...
const getConditions = async (): Promise<any[]> => {
  try {
    const { data, error } = await supabase
      .from('conditions')
      .select('*')
...
const addCondition = async (name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('conditions')
      .insert([{ name }])
...
const updateCondition = async (id: string, name: string): Promise<any | null> => {
  try {
    const { data, error } = await supabase
      .from('conditions')
      .update({ name })
